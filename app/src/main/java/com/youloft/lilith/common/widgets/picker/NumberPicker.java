...
public class NumberPicker extends View {
     ...
     */
    private static final int SELECTOR_MAX_FLING_VELOCITY_ADJUSTMENT = 10;
     ...
     */
    private static final float TOP_AND_BOTTOM_FADING_EDGE_STRENGTH = 1f;
...
//    private final PressedStateHelper mPressedStateHelper;
    ...
    public NumberPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        pl = attributesArray.getDimensionPixelSize(R.styleable.NumberPicker_pl, 0);
        ...
        detalSize = attributesArray.getDimensionPixelSize(R.styleable.NumberPicker_scaleDetal, 8);
        ...
        mTextSize = attributesArray.getDimensionPixelSize(R.styleable.NumberPicker_android_textSize, 28);
        int color = attributesArray.getColor(R.styleable.NumberPicker_android_textColor, Color.BLACK);
        mComputeMaxWidth = (mMaxWidth == SIZE_UNSPECIFIED);
        // mVirtualButtonPressedDrawable = attributesArray.getDrawable(
        // R.styleable.NumberPicker_virtualButtonPressedDrawable);
        mWrapSelectorWheel = attributesArray.getBoolean(R.styleable.NumberPicker_wheel, false);
        int gravity = attributesArray.getInt(R.styleable.NumberPicker_android_gravity, Gravity.CENTER);
...
//        setWillNotDraw(false);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(widthSize, Math.max(SELECTOR_WHEEL_ITEM_COUNT * mSelectionDividersDistance, heightSize));
...
//        System.out.println("eventaction:" + event.getAction());
...
//    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Align align = mSelectorWheelPaint.getTextAlign();
...
//            float t = (SELECTOR_MIDDLE_ITEM_INDEX - 1) * mSelectionDividersDistance;
//            float b = (SELECTOR_MIDDLE_ITEM_INDEX + 2) * mSelectionDividersDistance;
            float t = mTopSelectionDividerTop - mSelectionDividersDistance + mTextSize;
            float b = mBottomSelectionDividerBottom + mSelectionDividersDistance - mTextSize / 2;
            float c = (b - t) / 2 + t;

//            mSelectorWheelPaint.setColor(Color.BLUE);
//            canvas.drawLine(0, t, getWidth(), t, mSelectorWheelPaint);
//            canvas.drawLine(0, b, getWidth(), b, mSelectorWheelPaint);
//            canvas.drawLine(0, c, getWidth(), c, mSelectorWheelPaint);
            float top = y - mTextSize;
//            mSelectorWheelPaint.setColor(Color.RED);
//            canvas.drawLine(0, top, getWidth(), top, mSelectorWheelPaint);
            if (top < t || y > b) {
                mSelectorWheelPaint.setTextSize(mTextSize - detalSize);
            } else {
                if (top != c) {
                    float p = Math.abs((y - c) / mSelectorElementHeight);
                    mSelectorWheelPaint.setTextSize(mTextSize - Math.max(0, Math.min(p, 1)) * detalSize);
                } else {
...
//            float cp = 0;
            float ax = x;
//            if (top < mTopSelectionDividerTop) {
//                cp = (mTopSelectionDividerTop - top) / mTopSelectionDividerTop;
//                cp = (Math.max(0, Math.min(cp, 1)));
//                ax = ax + cp * 40;
//            } else if (y > mBottomSelectionDividerBottom) {
//                cp = (y - mBottomSelectionDividerBottom) / (getHeight() - mBottomSelectionDividerBottom);
//                cp = (Math.max(0, Math.min(cp, 1)));
//                ax = ax + cp * 40;
//            }
            ...
            if (align == Align.LEFT) {
                ...
                x = getWidth() - getPaddingRight();//(getRight() - getLeft()) - mSelectorWheelPaint.measureText(scrollSelectorValue);
            } else {
                x = (getRight() - getLeft() - pl - getPaddingRight()) / 2;
            }
            canvas.drawText(scrollSelectorValue, x, y, mSelectorWheelPaint);
//            canvas.drawText(mSelectorWheelPaint.getTextSize() + "", ax, y, mSelectorWheelPaint);
        ...
        if (mSelectionDivider != null) {
            ...
            mSelectionDivider.setBounds(0, topOfTopDivider, getWidth(),
                    bottomOfTopDivider);

            canvas.save();
            canvas.clipRect(mSelectionDivider.getBounds());
            ...
            mSelectionDivider.setBounds(0, topOfBottomDivider, getWidth(),
...
//        updateInputTextView();
    ...
    public void changeValueByOne(boolean increment) {
        if (mFlingScroller != null && !mFlingScroller.isFinished()) {
            return;
        }
    ...
    private void initializeSelectorWheel() {
        ...
        mSelectorElementHeight = mSelectionDividersDistance; // mTextSize + mSelectorTextGapHeight;
        Paint.FontMetrics fm = mSelectorWheelPaint.getFontMetrics();
        int editTextTextPosition = (int) ((getHeight() - fm.bottom - fm.top) / 2 + Math.round((fm.bottom - fm.descent)));
    ...
    private void invalidateValueCache() {
        ...
        int size = mSelectorIndexToStringCache.size();
        for (int i = 0; i < size; i++) {
            int key = mSelectorIndexToStringCache.keyAt(i);
            String value = formatNumber(key);
            mSelectorIndexToStringCache.put(key, value);
        }
...
//            NumberPicker.this.postDelayed(this, 350);
//        }
//
